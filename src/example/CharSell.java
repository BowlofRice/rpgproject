...
public class CharSell{
					...
					if(boxes.size()!=0){
						for(int i=0;i<boxes.size();i++){
							if(boxes.elementAt(i).isSelected()){
								...
								//mouse.allies.remove(i);
							}
						}
						
						for(int i=0;i<boxes2remove.size();i++){
							//System.out.println("boxes2remove size= "+boxes2remove.size());
							...
							//boxes.remove((int)boxes2remove.elementAt(i));
							//names.remove((int)boxes2remove.elementAt(i));
							//prices.remove((int)boxes2remove.elementAt(i));
							//mouse.allies.remove((int)boxes2remove.elementAt(i));
						}
						for(int i=0;i<boxes2remove.size();i++){
						...
						System.out.println("the size of boxes2remove is "+boxes2remove.size());
						System.out.println("the size of boxes is "+boxes.size());
						holder.revalidate();
						jd.repaint();
						jd.pack();
					}else{
	...
	public void makeUpWindow(Ally a){
			...
			//System.out.println(mouse.allies.size());
			names.add(name);
			prices.add(price);
			holder.add(name);
			holder.add(price);
